...
public class CursorOrders : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (SelectionTargetResolver.TryGetBestTarget(Camera.main, mousePos, out SelectionTargetResolver.TargetInfo targetInfo))
        {
            SelectableEntity selectable = targetInfo.selectable;
            bool hasUnitsSelected = (unitSelection != null && unitSelection.HasSelectedFriendlyUnits());

            if (selectable != null)
            {
                if (selectable.Category == SelectableEntity.SelectableCategory.EnemyUnit || selectable.Category == SelectableEntity.SelectableCategory.EnemyBuilding)
                {
                ...
                if (selectable.Category == SelectableEntity.SelectableCategory.Ground)
                {
                    if (hasUnitsSelected && mouse.rightButton.isPressed)
                ...
                if (selectable.Category == SelectableEntity.SelectableCategory.Building)
                {
                    BuildingState state = selectable.GetComponent<BuildingState>();
                ...
                if (selectable.Category == SelectableEntity.SelectableCategory.Resource)
                {
                    Cursor.SetCursor(cursorEmpanada, hotspot, CursorMode.ForceSoftware);
                    return;
                }
