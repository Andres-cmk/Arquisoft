...
public class SelectionBox : MonoBehaviour
{
    ...
    void FinalizarSeleccion()
    {
        ...
        if (Vector2.Distance(startMousePosition, endPos) > 10f)
        {
            if (selectionSystem != null)
            {
                ...
                GameObject[] allUnits = GameObject.FindGameObjectsWithTag("Unidad");
    ...
    void SeleccionIndividual()
    {
        ...
        {
            if (hit.collider.CompareTag("Unidad") || hit.collider.CompareTag("Edificio"))
            {
                selectionSystem.SelectUnit(hit.collider.gameObject);
                Debug.Log($"<color=white>[CLIC 3D]</color> Seleccionado: {hit.collider.name}");

                if (hit.collider.CompareTag("Edificio"))
                {
                    BuildingInteraction buildingUI = hit.collider.GetComponent<BuildingInteraction>();
                    if (buildingUI != null) buildingUI.ShowInspectionUI();
                }
            }
        }
    }
    ...
    void SeleccionDobleClic()
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit))
        {
            if (hit.collider.CompareTag("Unidad"))
            {
                GameObject[] allUnits = GameObject.FindGameObjectsWithTag("Unidad");
                foreach (GameObject unit in allUnits)
                {
                ...
                Debug.Log($"<color=white>[DOBLE CLIC]</color> Seleccionadas todas las unidades aliadas.");
            }
        }
    }
}